...
class LineBotUtils
{
    public static function createBot(): LINEBot
    {
        ...
        if ($accessToken == '') {
            $accessToken = (new self)->createAccessToken($accessToken, $channelSecret, $channelId);
        } else if (Carbon::parse($accessTokenExpireTime)->lessThanOrEqualTo(Carbon::now())) {
            (new self)->revokeAccessToken($accessToken);
            $accessToken = (new self)->createAccessToken($accessToken, $channelSecret, $channelId);
        }
